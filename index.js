...
function generateStringFromCodePoint(code1, code2, code3, code4) {
  return console.log(String.fromCodePoint(code1, code2, code3, code4));// Використовуємо String.fromCodePoint(), щоб створити рядок з символів code1, code2, code3, code4 на основі їх кодів Unicode
}
...
function rawString() {
  // Використовуємо , щоб створити рядок "Привіт\u000A!", який ігнорує спеціальні символи
...
function concatenateStrings(str1, str2) {
  let sum='';
  return sum.concat(str1, str2)// Використовуємо метод concat(), щоб об'єднати два рядки str1 та str2
}
...
function padString(original, length, padWith, side) {
  switch (side){// Перевіряємо, який бік було вказано
  case 'start':return original.padStart(padWith,length); break;// Якщо "start", використовуємо padStart
  // Використовуємо метод padStart(), щоб доповнити рядок string до певної довжини length, вставляючи символи padWith на початку
  case 'end':return original.padEnd(padWith,length); break;// Якщо "end", використовуємо padEnd
  // Використовуємо метод padEnd(), щоб доповнити рядок string до певної довжини length, вставляючи символи padWith на в кінці
  default:return console.log("Error: side should be either 'start' or 'end'");break;// Якщо бік не вказано або вказано неправильно, повертаємо повідомлення "Error: side should be either 'start' or 'end'"
}
}
...
function trimString(original, side) {
  switch(side){
    case 'start': return original.trimStart(); break;
    case 'end': return original.trimEnd(); break;
    case 'both': return original.trim(); break;
    default: return "Error: side should be either 'start', 'end' or 'both'"; break
  }
...
function convertCase(original, caseType) {
  switch(caseType){
    case 'upper': return original.toLowerCase(); break;
    case 'lower': return original.toUpperCase(); break;
    default:  return "Error: caseType should be either 'upper' or 'lower'"; break;
  }
